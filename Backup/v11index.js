  ...
  try {
    ...
    if (callbackData.startsWith("category|")) {
      ...
      if (functions.length > 0) {
        const buttons = [];
        for (let i = 0; i < functions.length; i += 3) {
          buttons.push([
            ...
            functions[i + 2] ? { text: functions[i + 2], callback_data: `function|${category}|${functions[i + 2]}` } : null
          ].filter(Boolean));
        }
        buttons.push([{ text: 'Back', callback_data: 'back_to_main' }]); // Add Back button
      ...
      console.log("Handling 'Close Menu' button, deleting the menu");
      await ctx.deleteMessage(); // Delete the message that contains the menu
    }
...
bot.on('message', async (ctx) => {
  ...
  if (userState && userState.waitingForBroadcastMessage) {
    // Ensure this is only possible in a direct message (DM)
    if (ctx.chat.type !== 'private') {
      console.log(`Broadcast attempt blocked in non-private chat: ${ctx.chat.id}`);
      await ctx.reply('Broadcast messages can only be sent in direct messages with the bot.');
      return;
    }
